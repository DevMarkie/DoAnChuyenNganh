...
def build_diagram_1_overall():
    ...
    add_cell(root, "b1", "HỆ THỐNG QUẢN LÝ SINH VIÊN (SMS)", STYLE_BOUNDARY, vertex=True, geometry={'x': 220, 'y': 30, 'width': 1050, 'height': 1050})

    # Actors
    add_cell(root, "act_u", "Người dùng\n(User)", STYLE_ACTOR, vertex=True, geometry={'x': 60, 'y': 70, 'width': 45, 'height': 80})
    add_cell(root, "act_a", "Quản trị viên\n(Admin)", STYLE_ACTOR, vertex=True, geometry={'x': 60, 'y': 400, 'width': 45, 'height': 80})
    add_cell(root, "act_l", "Giảng viên\n(Lecturer)", STYLE_ACTOR, vertex=True, geometry={'x': 60, 'y': 820, 'width': 45, 'height': 80})
    add_cell(root, "act_s", "Sinh viên\n(Student)", STYLE_ACTOR, vertex=True, geometry={'x': 1340, 'y': 500, 'width': 45, 'height': 80})
    ...
    ucs_admin = [
        ...
        ("uc_a_kpi", "Xem biểu đồ thống kê KPI", 260, 810, 220, 50),
    ...
    ucs_stu = [
        ...
        ("uc_s_trans", "Tra cứu Bảng điểm & Điểm CPA", 880, 690, 230, 50),
    ]
    ...
    add_cell(root, "inc_enr_search", "«include»", STYLE_INC, edge=True, source="uc_s_enr", target="uc_s_search", geometry={'relative': 1})
...
def build_diagram_2_admin():
    ...
    add_cell(root, "b2", "PHÂN HỆ QUẢN TRỊ VIÊN (ADMIN)", STYLE_BOUNDARY, vertex=True, geometry={'x': 220, 'y': 30, 'width': 1050, 'height': 1050})

    # Actor Admin
    add_cell(root, "act_a2", "Quản trị viên\n(Admin)", STYLE_ACTOR, vertex=True, geometry={'x': 60, 'y': 480, 'width': 45, 'height': 80})

    # Column 1: Core UCs (Starts at y=110, safely below boundary title)
    main_ucs = [
        ("uc2_kpi", "Xem biểu đồ thống kê KPI", 270, 110, 220, 50),
    ...
    add_cell(root, "inc_kpi1", "«include»", STYLE_INC, edge=True, source="uc2_kpi", target="uc2_sub_fac", geometry={'relative': 1})
    add_cell(root, "inc_kpi2", "«include»", STYLE_INC, edge=True, source="uc2_kpi", target="uc2_sub_stat", geometry={'relative': 1})
    ...
    add_cell(root, "ext_stu1", "«extend»", STYLE_EXT, edge=True, source="uc2_sub_stuedit", target="uc2_stu", geometry={'relative': 1})
    add_cell(root, "ext_stu2", "«extend»", STYLE_EXT, edge=True, source="uc2_sub_stustat", target="uc2_stu", geometry={'relative': 1})
    add_cell(root, "inc_stu3", "«include»", STYLE_INC, edge=True, source="uc2_stu", target="uc2_sub_stufilter", geometry={'relative': 1})
    ...
    add_cell(root, "inc_sec1", "«include»", STYLE_INC, edge=True, source="uc2_sec", target="uc2_sub_secass", geometry={'relative': 1})
    add_cell(root, "inc_sec2", "«include»", STYLE_INC, edge=True, source="uc2_sec", target="uc2_sub_seccfg", geometry={'relative': 1})
    add_cell(root, "ext_sec3", "«extend»", STYLE_EXT, edge=True, source="uc2_sub_secstat", target="uc2_sec", geometry={'relative': 1})
    ...
    add_cell(root, "inc_grd_view", "«include»", STYLE_INC, edge=True, source="uc2_mon", target="uc2_sub_viewgrd", geometry={'relative': 1})
...
def build_diagram_3_lecturer():
    ...
    # Boundary
    add_cell(root, "b3", "PHÂN HỆ GIẢNG VIÊN (LECTURER)", STYLE_BOUNDARY, vertex=True, geometry={'x': 220, 'y': 30, 'width': 980, 'height': 750})

    # Actor Lecturer
    add_cell(root, "act_l3", "Giảng viên\n(Lecturer)", STYLE_ACTOR, vertex=True, geometry={'x': 60, 'y': 340, 'width': 45, 'height': 80})

    # Column 1: Main UCs
    main_ucs = [
        ("uc3_dash", "Xem Dashboard Giảng viên", 270, 90, 240, 50),
        ("uc3_sch", "Tra cứu Lịch dạy theo tuần", 270, 180, 240, 50),
        ("uc3_sec", "Tra cứu Danh sách lớp phụ trách", 270, 280, 240, 50),
        ("uc3_grd", "Nhập điểm thành phần (CC, GK, CK)", 270, 410, 240, 55),
    ...
    add_cell(root, "inc_sec_stu", "«include»", STYLE_INC, edge=True, source="uc3_sec", target="uc3_sub_stulist", geometry={'relative': 1})
    ...
    add_cell(root, "inc_grd_calc", "«include»", STYLE_INC, edge=True, source="uc3_grd", target="uc3_sub_calc", geometry={'relative': 1})
    add_cell(root, "ext_grd_save", "«extend»", STYLE_EXT, edge=True, source="uc3_sub_save", target="uc3_grd", geometry={'relative': 1})
    add_cell(root, "ext_grd_fin", "«extend»", STYLE_EXT, edge=True, source="uc3_sub_fin", target="uc3_grd", geometry={'relative': 1})
...
def build_diagram_4_student():
    ...
    # Boundary
    add_cell(root, "b4", "PHÂN HỆ SINH VIÊN (STUDENT)", STYLE_BOUNDARY, vertex=True, geometry={'x': 220, 'y': 30, 'width': 980, 'height': 830})

    # Actor Student
    add_cell(root, "act_s4", "Sinh viên\n(Student)", STYLE_ACTOR, vertex=True, geometry={'x': 60, 'y': 380, 'width': 45, 'height': 80})

    # Column 1: Main UCs
    main_ucs = [
        ("uc4_dash", "Xem Dashboard tiến độ học tập", 270, 90, 240, 50),
    ...
    add_cell(root, "inc_enr_search", "«include»", STYLE_INC, edge=True, source="uc4_enr", target="uc4_sub_search", geometry={'relative': 1})
    add_cell(root, "inc_enr_val", "«include»", STYLE_INC, edge=True, source="uc4_enr", target="uc4_sub_val", geometry={'relative': 1})
    ...
    add_cell(root, "inc_drop_myenr", "«include»", STYLE_INC, edge=True, source="uc4_drop", target="uc4_sub_myenr", geometry={'relative': 1})

    # Transcript includes
    add_cell(root, "uc4_sub_gpa", "Xem GPA từng kỳ và CPA tích lũy", STYLE_UC_SUB, vertex=True, geometry={'x': 680, 'y': 550, 'width': 240, 'height': 45})
    add_cell(root, "uc4_sub_detail", "Xem chi tiết từng cột điểm (CC, GK, CK)", STYLE_UC_SUB, vertex=True, geometry={'x': 680, 'y': 615, 'width': 240, 'height': 45})
    add_cell(root, "inc_trans_gpa", "«include»", STYLE_INC, edge=True, source="uc4_trans", target="uc4_sub_gpa", geometry={'relative': 1})
    add_cell(root, "inc_trans_det", "«include»", STYLE_INC, edge=True, source="uc4_trans", target="uc4_sub_detail", geometry={'relative': 1})
...
def main():
    ...
    d1 = ET.SubElement(multi_file, 'diagram', {'id': 'diag_overall', 'name': '1. Tổng quan hệ thống (Overall)'})
    d1.append(build_diagram_1_overall())

    d2 = ET.SubElement(multi_file, 'diagram', {'id': 'diag_admin', 'name': '2. Phân hệ Admin'})
    ...
    save_single_diagram('2_UseCase_Admin.drawio', 'diag_admin', '2. Phân hệ Admin', build_diagram_2_admin)
